...
class ImprovedAIAssistant(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Assistente Virtual IA - Melhorada")
        self.resize(800, 600)
    ...
    def draw_neural_network(self, painter, center_x, center_y, scale):
        ...
        for layer_idx, nodes in enumerate(layers):
            for node in range(nodes):
                ...
                # Desenhar nó
                gradient = QRadialGradient(x, y, node_radius)
                ...
                if layer_idx < len(layers) - 1:
                    for next_node in range(layers[layer_idx + 1]):
                        ...
                        gradient.setColorAt(flow_offset / 100, QColor(100, 200, 255, 200))
                        gradient.setColorAt(min(1, (flow_offset + 20) / 100), QColor(0, 100, 200, 50))
    ...
    def paintEvent(self, event):
        ...
        for i in range(segments):
            ...
            pen = QPen(QColor(100, 200, 255, 100))
        ...
        # Efeito de brilho no "AI Assistant"
        for i in range(10):
            opacity = (10 - i) * 15
            y_offset = sin(self.pulse) * 3 * scale
            color = QColor(100, 200, 255, opacity)
            painter.setPen(color)
            painter.drawText(QRectF(center_x-150*scale, center_y-40*scale+y_offset-i/2*scale, 300*scale, 40*scale),
